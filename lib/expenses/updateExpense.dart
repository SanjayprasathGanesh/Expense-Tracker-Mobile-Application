...
class UpdateExpense extends StatefulWidget {
  ...
  const UpdateExpense({Key? key,required this.expenseModel}) : super(key: key);
...
class _UpdateExpenseState extends State<UpdateExpense> {
  ...
  void initState() {
    super.initState();
    exp_name.text = widget.expenseModel.exp_name!;
    exp_date.text = widget.expenseModel.exp_date!;
    exp_amt.text = widget.expenseModel.exp_amt!.toString();
    selectedType = widget.expenseModel.exp_type!;
    selectedTrans = widget.expenseModel.exp_trans!;
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Update Expenses",style: TextStyle(
          color: Colors.black,
          fontFamily: 'Poppins',
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        ElevatedButton(
                            ...
                            onPressed: () async{
                              setState(() {
                                validateName = exp_name.text.isEmpty;
                                validateDate = exp_date.text.isEmpty;
                                validateAmt = exp_amt.text.isEmpty;
                                validateType = selectedType.isEmpty;
                                validateTrans = selectedTrans.isEmpty;
                              });

                              if(!validateName && !validateDate && !validateAmt && !validateType && !validateTrans){

                                var expenseModel = ExpenseModel();
                                expenseModel.id = widget.expenseModel.id;
                                expenseModel.u_name = widget.expenseModel.u_name;
                                ...
                                if(result != null){
                                  ...
                                  Toast.show(
                                    "Sorry,Expense Not Updated Successfully",
                                    gravity: Toast.bottom,
                                    backgroundColor: Colors.red,
                                    duration: 3,
                                    textStyle: const TextStyle(
